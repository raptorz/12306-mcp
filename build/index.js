...
const server = new McpServer({
    ...
    instructions: '你是一个12306火车票务助手。你的主要任务是帮助用户查询火车票信息、特定列车的经停站信息以及相关的车站信息。请仔细理解用户的意图，并按以下指引选择合适的工具：\n\n' +
        '**核心场景：查询两地之间的火车票**\n' +
...
server.tool('get-current-date', '获取当前日期，以上海时区（Asia/Shanghai, UTC+8）为准，返回格式为 "yyyy-MM-dd"。主要用于解析用户提到的相对日期（如“明天”、“下周三”），以便为其他需要日期的工具（如 `get-tickets` 或 `get-train-route-stations`）提供准确的日期输入。', {}, async () => {
    try {
...
server.tool('get-stations-code-in-city', '通过中文城市名查询该城市 **所有** 火车站的名称及其对应的 `station_code`，结果是一个包含多个车站信息的列表。当用户想了解一个城市有哪些火车站，或者不确定具体从哪个车站出发/到达时可以使用此工具。', {
...
server.tool('get-station-code-of-city', '通过中文城市名（如“衡阳”）查询该城市 **主要或默认火车站** 的 `station_code` 和车站名，结果是唯一的。此工具主要用于在用户提供城市名作为出发地或到达地时，为 `get-tickets` 工具准备 `station_code` 参数。', {
...
server.tool('get-station-code-by-name', '通过具体的中文车站名（如“衡阳东站”）查询其 `station_code` 和车站名，结果是唯一的。此工具主要用于在用户提供具体车站名作为出发地或到达地时，为 `get-tickets` 工具准备 `station_code` 参数。', {
...
server.tool('get-station-by-telecode', '通过车站的 `station_telecode` (通常是3位字母编码) 查询车站的详细信息，包括名称、拼音、所属城市等。此工具主要用于在已知 `telecode` 的情况下获取更完整的车站数据，或用于特殊查询及调试目的。一般用户对话流程中较少直接触发。', {
...
server.tool('get-tickets', '查询12306余票信息。重要提示：调用此工具前，必须确保 `date` 参数是 "yyyy-MM-dd" 格式（若用户提供相对日期，先用 `get-current-date` 获取并计算），并且 `fromStation` 和 `toStation` 参数必须是 `station_code`（若用户提供中文地名，先用 `get-station-code-by-name` 或 `get-station-code-of-city` 获取）。', {
    ...
    trainFilterFlags: z
        ...
        .optional()
        .default('')
        .describe('车次筛选条件，默认为空。例如用户说“高铁票”，则应使用 "G"。可选标志：[G(高铁/城际),D(动车),Z(直达特快),T(特快),K(快速),O(其他),F(复兴号),S(智能动车组)]'),
...
server.tool('get-train-route-stations', '查询特定列车车次在指定区间内的途径车站、到站时间、出发时间及停留时间等详细经停信息。当用户询问某趟具体列车的经停站时使用此工具。', {
